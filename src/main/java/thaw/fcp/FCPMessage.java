...
/**
 * This class is a generic class, able to handle all kind of FCPMessage. Raw
 * data are NOT stored inside. You *have* to handle them by yourself
 * (FCPConnection.read() / FCPConnection.write()) after reading / writing a
...
public class FCPMessage {
	...
	public enum MessageType {
		 ...
		 *
		 * @param message
		 * 		Message name string as defined in http://new-wiki.freenetproject.org/FCPv2.
		 * @return The associated message enumeration value, if found.  If not found,
		 *         return null.
		 */
	...
	/**
	 * Raw message does not need to finish by "EndMessage" / "Data". Removes
	 * existing fields while loading in the new raw message. If null or a string
	 * with no valid fields is passed, the message will be empty.
	 */
	...
	/**
	 * Returns the first line number that contains a valid message identifier.  If
	 * a field is found (denoted by the "=" character) before a valid identifier is
	 * found, returns -1. If a valid identifier is not found, returns -1.
	 *
	 * @param messageLines
	 * 		ArrayList<String> containing the lines of a message. If null, does nothing
	 * 		and returns -1.
	 * @return The line number starting at 0 of the message identifier.  On error,
	 *         -1.
	 */
	...
	/**
	 * Extracts and stores fields from a message. Empty lines are ignored. Lines
	 * not containing '=' (like "Data" or "EndMessage") are ignored Lines with a
	 * leading '=' (like "=True") are malformed and are ignored.
	 *
	 * @param messageLines
	 * 		The message from which to extract the fields and values.  If null, does
	 * 		nothing.
	 * @param startLine
	 * 		The line number in messageLines to start searching from.
	 */
	 ...
	 *
	 * @return if > 0 : Data are still waiting (except if the message name is
	 *         "PersistentPut" !), if == 0 : No data waiting, if < 0 : These data
	 *         are now unavailable.
	 */
	...
	/**
	 * Generate FCP String to send. If amount of data waiting is set to > 0, then,
	 * a field "DataLength" is added, and resulting string finish by "Data", else
	 * resulting string simply finish by "EndMessage".
	 */
